...
static int cmp_func(const void *a, const void *b) {
	if (((const liz_chunkserver_info_t *)a)->ip < ((const liz_chunkserver_info_t *)b)->ip) {
		return -1;
	}
	if (((const liz_chunkserver_info_t *)a)->ip > ((const liz_chunkserver_info_t *)b)->ip) {
		return 1;
...
static int is_disconnected(const void *a, void *unused) {
	return ((const liz_chunkserver_info_t *)a)->version == kDisconnectedChunkserverVersion;
}
...
static int is_same_ip(const void *a, void *base) {
	...
	return ((const liz_chunkserver_info_t *)a)->ip == ((const liz_chunkserver_info_t *)a - 1)->ip;
}

static size_t remove_if(void *base, size_t num, size_t size,
                        int (*predicate)(const void *a, void *work_data), void *work_data) {
	size_t i, j;
	j = 0;
	for (i = 0; i < num; ++i) {
...
static liz_chunkserver_info_t *lzfs_int_get_randomized_chunkserver_list(
	...
	chunkserver_info = gsh_malloc(LZFS_BIGGEST_STRIPE_COUNT * sizeof(liz_chunkserver_info_t));

	rc = liz_get_chunkservers_info(lzfs_export->lzfs_instance, chunkserver_info,
	                               LZFS_BIGGEST_STRIPE_COUNT, chunkserver_count);
	if (rc < 0) {
		*chunkserver_count = 0;
	...
	*chunkserver_count = remove_if(chunkserver_info, *chunkserver_count,
	                               sizeof(liz_chunkserver_info_t), is_disconnected, NULL);

	// remove entries with the same ip
	qsort(chunkserver_info, *chunkserver_count, sizeof(liz_chunkserver_info_t), cmp_func);
	*chunkserver_count = remove_if(chunkserver_info, *chunkserver_count,
	                               sizeof(liz_chunkserver_info_t), is_same_ip, chunkserver_info);

	// randomize
	shuffle(chunkserver_info, *chunkserver_count, sizeof(liz_chunkserver_info_t));

	return chunkserver_info;
}

/*! \brief Fill DS list with entries corresponding to chunks */
static int lzfs_int_fill_chunk_ds_list(XDR *da_addr_body, liz_chunk_info_t *chunk_info,
                                       liz_chunkserver_info_t *chunkserver_info,
                                       uint32_t chunk_count, uint32_t stripe_count,
                                       uint32_t chunkserver_count, uint32_t *chunkserver_index) {
	fsal_multipath_member_t host[LZFS_EXPECTED_BACKUP_DS_COUNT];

	for (uint32_t chunk_index = 0; chunk_index < MIN(chunk_count, stripe_count); ++chunk_index) {
		liz_chunk_info_t *chunk = &chunk_info[chunk_index];
		int server_count = 0;
		memset(host, 0, LZFS_EXPECTED_BACKUP_DS_COUNT * sizeof(fsal_multipath_member_t));

		// prefer std chunk part type
		for (int i = 0; i < chunk->parts_size && server_count < LZFS_EXPECTED_BACKUP_DS_COUNT;
		     ++i) {
		...
		while (server_count < LZFS_EXPECTED_BACKUP_DS_COUNT) {
			host[server_count].proto = TCP_PROTO_NUMBER;
			host[server_count].addr = chunkserver_info[*chunkserver_index].ip;
			host[server_count].port = NFS_PORT;
			++server_count;
		...
		// encode ds entry
		nfsstat4 nfs_status = FSAL_encode_v4_multipath(da_addr_body, server_count, host);
...
static int lzfs_int_fill_unused_ds_list(XDR *da_addr_body, liz_chunkserver_info_t *chunkserver_info,
                                        uint32_t chunk_count, uint32_t stripe_count,
                                        uint32_t chunkserver_count, uint32_t *chunkserver_index) {
	fsal_multipath_member_t host[LZFS_EXPECTED_BACKUP_DS_COUNT];

	for (uint32_t chunk_index = MIN(chunk_count, stripe_count); chunk_index < stripe_count;
	     ++chunk_index) {
		int server_count = 0, index;
		memset(host, 0, LZFS_EXPECTED_BACKUP_DS_COUNT * sizeof(fsal_multipath_member_t));

		while (server_count < LZFS_EXPECTED_BACKUP_DS_COUNT) {
			index = (*chunkserver_index + server_count) % chunkserver_count;
			host[server_count].proto = TCP_PROTO_NUMBER;
 ...
 */
static nfsstat4 lzfs_fsal_getdeviceinfo(struct fsal_module *fsal_hdl, XDR *da_addr_body,
	...
	chunkserver_index = 0;
	stripe_count = MIN(chunk_count + chunkserver_count, LZFS_BIGGEST_STRIPE_COUNT);
	if (!inline_xdr_u_int32_t(da_addr_body, &stripe_count)) {
		goto encode_err;
	}

	for (uint32_t chunk_index = 0; chunk_index < stripe_count; ++chunk_index) {
	...
	rc = lzfs_int_fill_chunk_ds_list(da_addr_body, chunk_info, chunkserver_info, chunk_count,
	                                 stripe_count, chunkserver_count, &chunkserver_index);
	if (rc < 0) {
		goto encode_err;
	}

	rc = lzfs_int_fill_unused_ds_list(da_addr_body, chunkserver_info, chunk_count, stripe_count,
 ...
 * \see fsal_api.h for more information
 */
static nfsstat4 lzfs_fsal_getdevicelist(struct fsal_export *export_hdl, layouttype4 type,
 ...
 * \see fsal_api.h for more information
 */
static void lzfs_fsal_fs_layouttypes(struct fsal_export *export_hdl, int32_t *count,
...
static size_t lzfs_fsal_fs_da_addr_size(struct fsal_module *fsal_hdl) {
	// one stripe index + number of addresses + LZFS_EXPECTED_BACKUP_DS_COUNT addresses per chunk
	// each address takes 37 bytes (we use 40 for safety)
	// we add 32 bytes of overhead (includes stripe count and DS count)
	return LZFS_BIGGEST_STRIPE_COUNT * (4 + (4 + LZFS_EXPECTED_BACKUP_DS_COUNT * 40)) + 32;
}
